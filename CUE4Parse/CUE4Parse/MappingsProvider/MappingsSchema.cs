...
namespace CUE4Parse.MappingsProvider
{
    public class Struct
    {
        ...
        {
            info = new PropertyInfo(-1, string.Empty, new PropertyType("None"));
            if (Properties.TryGetValue(i, out var tempInfo))
            {
                info = tempInfo;
                return true;
            }
            ...
            {
                bool result = Super.Value.TryGetValue(i - PropertyCount, out var superInfo);
                if (result && superInfo != null)
                {
                    info = superInfo;
                }
                return result;

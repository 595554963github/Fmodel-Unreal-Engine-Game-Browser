...
namespace CUE4Parse.FileProvider.Vfs
{
    public abstract class AbstractVfsFileProvider : AbstractFileProvider, IVfsFileProvider
    {
        ...
        protected AbstractVfsFileProvider(VersionContainer? versions = null, StringComparer? pathComparer = null) : base(versions, pathComparer)
        {
            ...
            {
                EGame.GAME_Apex英雄移动版 => ApexLegendsMobileAes.DecryptApexMobile,
                EGame.GAME_尘白禁区 => SnowbreakAes.SnowbreakDecrypt,
                EGame.GAME_漫威争锋 => MarvelAes.MarvelDecrypt,
                EGame.GAME_黎明觉醒 => ToaaAes.ToaaDecrypt,
                EGame.GAME_黎明杀机 => DBDAes.DbDDecrypt,
                EGame.GAME_天域安宁 => PaxDeiAes.PaxDeiDecrypt,
                EGame.GAME_街头篮球_反弹 => FreeStyleReboundAes.FSRDecrypt,
                EGame.GAME_元梦之星 => DreamStarAes.DreamStarDecrypt,
                EGame.GAME_三角洲行动 => DeltaForceAes.DeltaForceDecrypt,
                EGame.GAME_达愿福神社 => PMAAes.PMADecrypt,
                EGame.GAME_怪物卡车对决 => MonsterJamShowdownAes.MonsterJamShowdownDecrypt,
                EGame.GAME_Rennsport => RennsportAes.RennsportDecrypt,
                EGame.GAME_公仔总动员 => FunkoFusionAes.FunkoFusionDecrypt,
                EGame.GAME_托尼霍克职业滑板12 => THPS12Aes.THPS12Decrypt,
                _ => null
            };
        }
        ...
        private void PostLoadReader(AbstractAesVfsReader reader, bool isConcurrent = true)
        {
            ...
            if (!(reader.Game == EGame.GAME_漫威争锋 && reader is IoStoreReader)) // no custom encryption for MR IoStore
            {
        ...
        public async Task<int> SubmitKeysAsync(IEnumerable<KeyValuePair<FGuid, FAesKey>> keys)
        {
            ...
            foreach (var (guid, key) in keys)
            {
                ...
                {
                    if (reader.Game == EGame.GAME_界外狂潮 && reader.Name.Contains("global")) reader.AesKey = key;
                    VerifyGlobalData(reader);
        ...
        public IAesVfsReader GetArchive(string archiveName, StringComparison comparison = StringComparison.Ordinal)
        {
            ...
            return MountedVfs.FirstOrDefault(predicate) ??
                   ...
                   throw new KeyNotFoundException($"没有包含名称的存档文件\"{archiveName}\"");
        }
        ...
        public GameFile this[string path, IAesVfsReader archive]
            => TryGetGameFile(path, archive.Files, out var file)
                ? file
                : throw new KeyNotFoundException($"没有带路径的游戏文件\"{path}\"在\"{archive.Name}\"");

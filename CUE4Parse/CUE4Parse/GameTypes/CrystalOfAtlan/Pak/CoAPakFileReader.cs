...
public partial class PakFileReader
{
    ...
    private void CoAReadIndexUpdated(StringComparer pathComparer)
    {
        ...
        {
            throw new InvalidAesKeyException($"给定的aes密钥'{AesKey?.KeyString}'不工作'{Name}'", e);
        }
        ...
        var encodedEntries = new FByteArchive("编码条目", encodedPakEntries);
        ...
        if (directoryIndexLength == 0)
        {
            ...
            foreach (var key in pathHashIndex)
            {
                ...
                else
                {
                    ...
                    {
                        Log.Warning("找不到{0}路径哈希的包名称", hash);
                        continue;
                    }
                }
                ...
                var name = package.NameMap.FirstOrDefault(name => pathComparer.Equals(name.Name?.SubstringAfterLast('/') ?? string.Empty, assetname) && (name.Name?.StartsWith('/') ?? false));
                ...
                void FindPayload(FArchive Ar, string extension, bool warning = false)
                {
                    ...
                    {
                        Log.Warning("包{1}缺少{0}文件", extension, packageName);
                    }
                }
            ...
            foreach (var hash in pathHashIndex.Keys.Except(used))
            {
                ...
                if (entry.IsEncrypted)
                    EncryptedFileCount++;
                files[path] = entry;
                Log.Warning("找不到{0}路径哈希的对应名称", hash);
            }

...
namespace CUE4Parse.GameTypes.FN.Assets.Exports
{
    ...
    public class FActorTemplateRecord
    {
        ...
        public FActorTemplateRecord(FLevelSaveRecordArchive Ar)
        {
            ...
            else
            {
                ...
                if (DataHash != hash)
                {
                    Log.Error("FActorTemplateRecord::Serialize未能反序列化数据:{0}删除损坏的数据.", ActorClass.ToString());
                    ActorData = null;
                    DataHash = 0;
                }
            }

            // Initialize ActorDataReferenceTable and ActorDataHardReferences to avoid null errors
            ActorDataReferenceTable = Array.Empty<FSoftObjectPath>();
            ActorDataHardReferences = Array.Empty<bool>();
        }
        ...
        public FStructFallback ReadActorData(IPackage owner, ELevelSaveRecordVersion SaveVersion)
        {
            ...
            {
                var Ar = new FLevelSaveRecordArchive(new FAssetArchive(new FByteArchive("ActorData读取器", ActorData), owner), SaveVersion);
    ...
    public class FActorComponentRecord
    {
        ...
        public FActorComponentRecord(FLevelSaveRecordArchive Ar)
        {
            ...
            else
            {
                ...
                if (DataHash != hash)
                {
                    Log.Error("FActorTemplateRecord::Serialize 未能反序列化数据:{0}删除损坏的数据.", ComponentClass.ToString());
                    ComponentData = null;
                    DataHash = 0;
                }
            }
    ...
    public class FFvkModuleVersionRef
    {
    ...
    public class FFortCreativeVkPalette_ProjectInfo
    {
        ...
        public int unk;
        public required FFvkModuleVersionRef[] PublicModules;

        public FFortCreativeVkPalette_ProjectInfo(FArchive Ar)
        {
            LinkVersion = Ar.Read<int>();
            PublicModules = Array.Empty<FFvkModuleVersionRef>();
        }
    }

    public class FFortCreativeVkPalette
{
    public required Dictionary<string, FFortCreativeVkPalette_ProjectInfo> LinkCodeMap { get; set; }

    public FFortCreativeVkPalette()
    {
        LinkCodeMap = new Dictionary<string, FFortCreativeVkPalette_ProjectInfo>();
    }
    ...
    public class ULevelSaveRecord : UObject
    {
        ...
        public Dictionary<int, FActorTemplateRecord> TemplateRecords { get; set; } = new();
        public Dictionary<FGuid, FActorInstanceRecord> ActorInstanceRecords { get; set; } = new();
        public Dictionary<FGuid, FActorInstanceRecord> VolumeInfoActorRecords { get; set; } = new();
        public Dictionary<FName, FLevelStreamedDeleteActorRecord> LevelStreamedActorsToDelete { get; set; } = new();
        public int PlayerPersistenceUserWipeNumber;
        public FFortCreativeVkPalette VkPalette { get; set; } = new FFortCreativeVkPalette
        {
            LinkCodeMap = new Dictionary<string, FFortCreativeVkPalette.FFortCreativeVkPalette_ProjectInfo>()
        };
        public required string IslandTemplateId; // UnknownData03[0x48]
        public byte NavmeshRequired; // TODO Find out its enum values
        public bool bRequiresGridPlacement;
        public required List<FStructFallback> ActorData;
        ...
        public override void Deserialize(FAssetArchive Ar, long validPos)
        {
            ...
            else
            {
                ...
                foreach (var kv in GetOrDefault<UScriptMap>("TemplateRecords").Properties)
                {
                    ...
                    if (templateRecord == null) continue;
                    ...
                    {
                        ActorData.Add(new FStructFallback());
                    }
                }
            }
        }
        ...
        private void DeserializeLevelSaveRecordData(FLevelSaveRecordArchive Ar)
        {
            ...
            {
                VkPalette = new FFortCreativeVkPalette
                {
                    LinkCodeMap = new Dictionary<string, FFortCreativeVkPalette.FFortCreativeVkPalette_ProjectInfo>()
                };
            }

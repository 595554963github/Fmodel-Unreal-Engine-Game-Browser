...
namespace CUE4Parse.UE4.Assets
{
    ...
    public sealed class IoPackage : AbstractUePackage
    {
        ...
        public readonly FBulkDataMapEntry[] BulkDataMap = Array.Empty<FBulkDataMapEntry>();
        ...
        {
            _globalData = provider?.GlobalData ?? throw new ParserException("找到IoStore包，但全局数据丢失，无法序列化");
            ...
            if (uassetAr.Game >= EGame.GAME_UE5_0)
            {
                ...
                if (containerHeader != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Log.Warning("找不到包{0}的存储条目，其数据将不会被完全读取", Name);
                        }
                    }
                ...
                BulkDataMap = [];
                if (uassetAr.Ver >= EUnrealEngineObjectUE5Version.DATA_RESOURCES || uassetAr.Game == EGame.GAME_第一后裔)
                {
        ...
        public ResolvedObject? ResolveObjectIndex(FPackageObjectIndex index)
        {
            ...
            {
                Log.Warning("缺少包{2}的导入项{0}0x{1:X}", index.IsScriptImport ? "script" : "package", index.Value, Name);
            }

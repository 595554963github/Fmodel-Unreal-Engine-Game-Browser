...
public abstract class AbstractUePackage : UObject, IPackage
{
    ...
    public bool CanDeserialize
    {
        get
        {
            if (HasFlags(EPackageFlags.PKG_UnversionedProperties) && Mappings is null)
                throw new ParserException("包具有未版本化的属性，但缺少映射文件，无法序列化");
            return true;
        }
    }
    ...
    protected AbstractUePackage(string name, IFileProvider? provider)
    {
        ...
        ExportsLazy = Array.Empty<Lazy<UObject>>(); // Initialize ExportsLazy with an empty array
    }
    ...
    protected void DeserializeObject(UObject obj, FAssetArchive Ar, long serialSize)
    {
        ...
        catch (Exception e)
        {
            ...
            {
                throw new ParserException($"无法正确读取{obj.ExportType}", e);
            }
            Log.Error(e, "无法正确读取{0}", obj.ExportType);
        }
    }
...
public abstract class ResolvedObject : IObject
{
    ...
    {
        resultString.Append(includeClassPackage ? Class?.GetPathName() ?? "None" : Class?.Name.ToString() ?? "None");
        resultString.Append('\'');
...
public class ResolvedLoadedObject : ResolvedObject
{
    ...
    public ResolvedLoadedObject(UObject obj) : base(obj.Owner ?? throw new ArgumentNullException(nameof(obj.Owner)))
    {
        _object = obj;
    }

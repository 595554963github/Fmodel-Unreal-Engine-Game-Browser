...
public class FPropertyTag
{
    ...
    public FPropertyTag(FAssetArchive Ar, PropertyInfo info, ReadType type)
    {
        ...
        {
            throw new ParserException($"∂¡»°FPropertyTagType ß∞‹{TagData?.ToString() ?? PropertyType.Text} {Name.Text}", e);
        }
    ...
    public override string ToString() => $"{Name.Text}  -->  {Tag?.ToString() ?? "Ω‚Œˆ ß∞‹"}";
}
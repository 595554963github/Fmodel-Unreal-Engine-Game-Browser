...
public class FScriptStruct
{
    ...
    public FScriptStruct(FAssetArchive Ar, string? structName, UStruct? struc, ReadType? type)
    {
        StructType = structName switch
        {
            ...
            // GTA: The Trilogy
            "ScalarParameterValue" when Ar.Game == EGame.GAME_ÏÀµÁÁÔ³µÊÖ_Èý²¿Çú_ÖÕ¼¶°æ => new FScalarParameterValue(Ar),
            "VectorParameterValue" when Ar.Game == EGame.GAME_ÏÀµÁÁÔ³µÊÖ_Èý²¿Çú_ÖÕ¼¶°æ => new FVectorParameterValue(Ar),
            "TextureParameterValue" when Ar.Game == EGame.GAME_ÏÀµÁÁÔ³µÊÖ_Èý²¿Çú_ÖÕ¼¶°æ => new FTextureParameterValue(Ar),
            "MaterialTextureInfo" when Ar.Game == EGame.GAME_ÏÀµÁÁÔ³µÊÖ_Èý²¿Çú_ÖÕ¼¶°æ => new FMaterialTextureInfo(Ar),
            ...
            // Gothic 1 Remake
            "WaynetNode" when Ar.Game == EGame.GAME_¸çÌØÍõ³¯ÖØÖÆ°æ => new FWaynetNode(Ar),
            "WaynetPath" when Ar.Game == EGame.GAME_¸çÌØÍõ³¯ÖØÖÆ°æ => new FWaynetPath(Ar),
            ...
            // Tempest Rising
            "OffsetCoords" when Ar.Game == EGame.GAME_·ç±©áÈÆð => type == ReadType.ZERO ? new TIntVector2<float>() : Ar.Read<TIntVector2<float>>(),
            ...
            // State of Decay 2
            "ItemsBitArray" when Ar.Game == EGame.GAME_¸¯ÀÃ¹ú¶È2 => type == ReadType.ZERO ? new FItemsBitArray() : new FItemsBitArray(Ar),

            // Dune Awakening
            "BodyInstance" when Ar.Game == EGame.GAME_É³Çð_¾õÐÑ => new FBodyInstance(Ar),
            "GenericTeamId" when Ar.Game == EGame.GAME_É³Çð_¾õÐÑ => new FGenericTeamId(Ar),
            "UniqueID" when Ar.Game == EGame.GAME_É³Çð_¾õÐÑ => new FUniqueID(Ar),
            "BotAutoBorderCrossingConfig" when Ar.Game == EGame.GAME_É³Çð_¾õÐÑ => new FBotAutoBorderCrossingConfig(Ar),

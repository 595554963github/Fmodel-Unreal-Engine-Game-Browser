...
public class FKismetArchive : FArchive
{
    ...
    public KismetExpression ReadExpression()
    {
        ...
        KismetExpression expression = token switch
        {
            ...
            _ => throw new ParserException($"未知的EExprToken {token}")
        };
    ...
    public string XFERSTRING()
    {
        ...
        if (eos == -1) throw new ParserException("无法找到字符串的结束位置");
    ...
    public string XFERUNICODESTRING()
    {
        ...
        if (length == -1) throw new ParserException("无法找到Unicode字符串的结束位置");
    ...
    public override FName ReadFName()
    {
        ...
        {
            throw new ParserException(this, $"无法读取FName，请求的索引{nameIndex}超出名称映射范围{Owner.NameMap.Length}");
        }
#endif
    ...
    {
        int n = Math.Min((int)(Length - Position), count);
        if (n <= 0)
            return 0;
        ...
        if (n <= 8)
        {
            for (int i = 0; i < n; i++)
            {
                buffer[offset + i] = _data[(int)Position + i];
            }
        }
        else
        {
    ...
    {
        if (length < 0)
            throw new ArgumentOutOfRangeException(nameof(length), "读取长度不能为负数");

        var result = new byte[length];
        var bytesRead = Read(result, 0, length);
        if (bytesRead < length)
        {
    ...
    public override long Seek(long offset, SeekOrigin origin)
    {
        Position = origin switch
        {
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(origin), "不支持的SeekOrigin类型")
        };
        return Position;
    }
    ...
    public override T Read<T>()
    {
        ...
        if (Position + size > Length)
            throw new EndOfStreamException("已到达流末尾");

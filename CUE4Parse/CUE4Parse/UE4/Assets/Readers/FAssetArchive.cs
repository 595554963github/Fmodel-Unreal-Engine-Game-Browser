...
namespace CUE4Parse.UE4.Assets.Readers
{
    public class FAssetArchive : FArchive
    {
        ...
        public override FName ReadFName()
        {
            ...
            {
                throw new ParserException(this, $"无法读取FName，请求的索引 {nameIndex}，名称映射大小 {Owner!.NameMap.Length}");
            }
#endif
        ...
        public virtual Lazy<T?> ReadObject<T>() where T : UObject
        {
            ...
            return new Lazy<T?>(() =>
            {
                if (resolved == null)
                {
                    ...
                    Log.Warning("无法解析索引{Index}", index);
                    return null;
                }

                if (Owner?.Provider == null)
                {
                    Log.Warning("没有文件提供程序，无法加载对象{Resolved}", resolved.Name);
                    return null;
                }

                if (!resolved.TryLoad(out var obj))
                {
                    Log.Warning("加载对象{Obj}失败", resolved.Name);
                    return null;
                }

                if (obj != null && obj is T cast)
                {
                    return cast;
                }
                Log.Warning("对象类型为{ObjType},与预期类型{Type}不匹配", obj?.GetType().Name ?? "null", typeof(T).Name);
        ...
        public FAssetArchive GetPayload(PayloadType type)
        {
            ...
            return reader ?? throw new ParserException(this, $"未找到类型为{type}的请求负载");
        }
        ...
        public void AddPayload(PayloadType type, FAssetArchive payload)
        {
            ...
            {
                throw new ParserException(this, $"无法添加已存在的类型为{type}的负载");
            }

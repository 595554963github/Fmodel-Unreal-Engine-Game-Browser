...
public class FStaticMeshLODResources
{
    ...
    public FStaticMeshLODResources(FArchive Ar)
    {
        ...
        if (Ar.Game == EGame.GAME_全境封锁_曙光) Ar.Position += 4;

        Sections = Ar.ReadArray(() => new FStaticMeshSection(Ar));
        MaxDeviation = Ar.Read<float>();

        if (Ar.Game == EGame.GAME_无路之旅) Ar.Position += 4;
        ...
        var bInlined = Ar.ReadBoolean() || Ar.Game == EGame.GAME_侠盗公司;
        ...
        if (!stripDataFlags.IsAudioVisualDataStripped() && !bIsLODCookedOut)
        {
            ...
            if (bInlined)
            {
                ...
                switch (Ar.Game)
                {
                    case EGame.GAME_侠盗公司:
                        Ar.Position += 10;
                        break;
                    case EGame.GAME_全境封锁_曙光:
                        Ar.Position += 12;
                        break;
                }
            }
            else if (Ar is FAssetArchive assetArchive)
            {
                ...
                if (Ar.Game is EGame.GAME_星球大战绝地_幸存者 or EGame.GAME_三角洲行动) Ar.Position += 4; // bDropNormals
            }
    ...
    public void SerializeBuffersLegacy(FArchive Ar, FStripDataFlags stripDataFlags)
    {
        ...
        if (Ar.Game != EGame.GAME_绝地求生大逃杀 || !stripDataFlags.IsClassDataStripped((byte) EClassDataStripFlag.CDSF_StripIndexBuffers))
        {
        ...
        if (Ar.Game == EGame.GAME_盗贼之海) Ar.Position += 17;
    }
    ...
    public void SerializeBuffers(FArchive Ar)
    {
        ...
        if (Ar.Game == EGame.GAME_无路之旅) Ar.Position += 20;

...
public class FStaticMeshRenderData
{
    ...
    public FStaticMeshRenderData(FAssetArchive Ar)
    {
        ...
        {
            EGame.GAME_HYENAS_直译为鬣狗 => 1,
            EGame.GAME_黎明觉醒 => 4,
            EGame.GAME_往日不再 => Ar.Read<int>() * 4,
            _ => 0
        };

        if (Ar.Game == EGame.GAME_黎明觉醒)
        {
        ...
        if (Ar.Game is EGame.GAME_三角洲行动 or EGame.GAME_死域Rogue) Ar.Position += 4;
        if (Ar.Game is EGame.GAME_无限暖暖) Ar.Position += 8;

        var screenSizeLength = Ar.Game switch
        {
            EGame.GAME_界外狂潮 => 16,
            EGame.GAME_潜行者2_切尔诺贝利之心 => 14,
        ...
        for (var i = 0; i < ScreenSize.Length; ++i)
        {
            ...
            if (Ar.Game == EGame.GAME_霍格沃茨遗产) Ar.Position += 8;
            if (Ar.Game == EGame.GAME_圣剑传说) Ar.Position += 4;
        }

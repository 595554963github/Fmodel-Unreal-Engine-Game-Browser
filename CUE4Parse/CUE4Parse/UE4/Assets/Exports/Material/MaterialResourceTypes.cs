...
public class FMaterial
{
    ...
    public void DeserializeInlineShaderMap(FMaterialResourceProxyReader Ar)
    {
        ...
        if (bValid)
        {
            ...
            if (Ar.Game == EGame.GAME_潜行者2_切尔诺贝利之心) Ar.Position += 8;
        }
        else
        {
            Log.Warning("使用无效的着色器映射加载材质资源'{0}'!", Ar.Name);
        }
    }
}
...
public class FShaderMapContent
{
    ...
    public FShaderMapContent(FMemoryImageArchive Ar)
    {
        ...
        if (Ar.Game >= EGame.GAME_UE5_2)
        {
            ...
            if (Ar.Game == EGame.GAME_漫威争锋) Ar.Position += 8;
        }
        else
        {
            ShaderPlatform = Ar.Read<EShaderPlatform>();
...
public class FUniformExpressionSet
{
    ...
    public FMaterialUniformParameterEvaluation[] UniformParameterEvaluations = Array.Empty<FMaterialUniformParameterEvaluation>();
    public FMaterialUniformPreshaderHeader[] UniformPreshaders = Array.Empty<FMaterialUniformPreshaderHeader>();
    public FMaterialUniformPreshaderField[]? UniformPreshaderFields = null;
    public FMaterialNumericParameterInfo[] UniformNumericParameters = Array.Empty<FMaterialNumericParameterInfo>();
    public FMaterialTextureParameterInfo[][] UniformTextureParameters = Array.Empty<FMaterialTextureParameterInfo[]>();
    public FMaterialExternalTextureParameterInfo[] UniformExternalTextureParameters = Array.Empty<FMaterialExternalTextureParameterInfo>();
    public FMaterialTextureCollectionParameterInfo[] UniformTextureCollectionParameters = Array.Empty<FMaterialTextureCollectionParameterInfo>();
    public uint UniformPreshaderBufferSize;
    public FMaterialPreshaderData UniformPreshaderData;
    public byte[]? DefaultValues = null;
    public FMaterialVirtualTextureStack[] VTStacks = Array.Empty<FMaterialVirtualTextureStack>();
    public FGuid[] ParameterCollections = Array.Empty<FGuid>();
    ...
    public FUniformExpressionSet(FMemoryImageArchive Ar)
    {
        ...
        if (Ar.Game >= EGame.GAME_UE5_0)
        {
            ...
            UniformTextureCollectionParameters = Ar.Game >= EGame.GAME_UE5_5 ? Ar.ReadArray(() => new FMaterialTextureCollectionParameterInfo(Ar)) : Array.Empty<FMaterialTextureCollectionParameterInfo>();
            ...
            foreach (var parameter in UniformNumericParameters)
            {
                ...
                parameter.Value = parameter.ParameterType switch
                {
                    ...
                    _ => throw new NotImplementedException($"未知的EMaterialParameterType : {parameter.ParameterType}"),
                };
            }

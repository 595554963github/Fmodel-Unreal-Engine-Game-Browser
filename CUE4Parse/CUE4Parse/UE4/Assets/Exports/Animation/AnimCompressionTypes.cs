...
namespace CUE4Parse.UE4.Assets.Exports.Animation
{
    public class FCompressedOffsetDataBase<T> where T : struct
    {
        ...
        {
            OffsetData = Ar.ReadArray<T>() ?? Array.Empty<T>();
    ...
    public class FCompressedAnimDataBase
    {
         ...
         *   . . .
         */
        public int[] CompressedTrackOffsets { get; protected set; } = Array.Empty<int>();
         ...
         *   . . .
         */
        public FCompressedOffsetData CompressedScaleOffsets { get; protected set; } = new();

        public byte[] CompressedByteStream { get; protected set; } = Array.Empty<byte>();
        ...
        protected FCompressedAnimDataBase()
        {
            // Initialize with empty arrays to ensure non-null values
            CompressedTrackOffsets = Array.Empty<int>();
            CompressedByteStream = Array.Empty<byte>();
        }
    }
    ...
    public class FUECompressedAnimData : FCompressedAnimDataBase, ICompressedAnimData
    {
        ...
        public FUECompressedAnimData()
        {
            // Ensure base initialization
        }
        ...
        public void InitViewsFromBuffer(byte[] bulkData)
        {
            ...
            {
                CompressedTrackOffsets = Array.Empty<int>();
                CompressedScaleOffsets.OffsetData = Array.Empty<int>();
                CompressedByteStream = Array.Empty<byte>();
                return;
            }

            using var tempAr = new FByteArchive("SerializedByteStream", bulkData);
            CompressedTrackOffsets = tempAr.ReadArray<int>() ?? Array.Empty<int>();
            CompressedScaleOffsets.OffsetData = tempAr.ReadArray<int>() ?? Array.Empty<int>();
            CompressedByteStream = tempAr.ReadArray<byte>() ?? Array.Empty<byte>();
        }
        ...
        public void Bind(byte[] bulkData) => InitViewsFromBuffer(bulkData ?? Array.Empty<byte>());

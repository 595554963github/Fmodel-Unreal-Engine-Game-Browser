...
public class FTexturePlatformData
{
    ...
    public FTexturePlatformData(FAssetArchive Ar, UTexture Owner)
    {
        ...
        PixelFormat = Ar.Game == EGame.GAME_战争机器4 ? Ar.ReadFName().Text : Ar.ReadFString();

        if (Ar.Game == EGame.GAME_最终幻想7重制版 && (PackedData & 0xffff) == 16384)
        {
        ...
        if (HasOptData())
        {
            if (Ar.Game == EGame.GAME_午夜太阳) Ar.Position += 4;
        ...
        if (Ar.Game == EGame.GAME_往日不再) Ar.Position += 8;

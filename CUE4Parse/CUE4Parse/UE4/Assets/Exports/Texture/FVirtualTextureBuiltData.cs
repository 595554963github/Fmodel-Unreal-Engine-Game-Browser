...
public readonly struct FVirtualTextureTileOffsetData : IUStruct
{
    ...
    public FVirtualTextureTileOffsetData(FArchive Ar)
    {
        ...
        Addresses = Ar.ReadArray<uint>() ?? Array.Empty<uint>();
        Offsets = Ar.ReadArray<uint>() ?? Array.Empty<uint>();
    }
    ...
    public uint GetTileOffset(uint inAddress)
    {
        ...
        if (blockIndex < 0 || blockIndex >= Offsets.Length)
            return ~0u;
        ...
        uint baseAddress = blockIndex < Addresses.Length ? Addresses[blockIndex] : 0;
...
public class FVirtualTextureBuiltData
{
    ...
    public FVirtualTextureBuiltData(FAssetArchive Ar, int firstMip)
    {
        ...
        {
            ChunkIndexPerMip = null;
            BaseOffsetPerMip = null;
            TileOffsetData = null;
        }

        TileIndexPerChunk = Ar.ReadArray<uint>() ?? Array.Empty<uint>();
        TileIndexPerMip = Ar.ReadArray<uint>() ?? Array.Empty<uint>();
        TileOffsetInChunk = Ar.ReadArray<uint>() ?? Array.Empty<uint>();

        LayerTypes = Ar.ReadArray((int)NumLayers, () => (EPixelFormat)Enum.Parse(typeof(EPixelFormat), Ar.ReadFString()))
            ?? Array.Empty<EPixelFormat>();
        ...
        {
            LayerFallbackColors = Array.Empty<FLinearColor>();
        }

        Chunks = Ar.ReadArray(() => new FVirtualTextureDataChunk(Ar, NumLayers))
            ?? Array.Empty<FVirtualTextureDataChunk>();
    }
    ...
    public bool IsLegacyData() => TileOffsetInChunk.Length > 0;
    public int GetNumTileHeaders() => TileOffsetInChunk.Length;
    ...
    public int GetChunkIndex_Legacy(uint tileIndex)
    {
        ...
        if (TileIndexPerChunk.Length > 0 && tileIndex <= TileIndexPerChunk.Last())
        {
            for (var i = 0; i < max; i++)
            {
    ...
    public bool IsValidAddress(int vLevel, uint vAddress)
    {
        ...
        else if (TileOffsetData != null && vLevel < TileOffsetData.Length)
        {
    ...
    public (int, uint, uint) GetTileData(int vLevel, uint vAddress, uint layerIndex)
    {
        ...
        else if (BaseOffsetPerMip != null && TileOffsetData != null &&
                 vLevel < BaseOffsetPerMip.Length && vLevel < TileOffsetData.Length)
        {
            ...
            if (baseOffset != ~0u && tileOffset != ~0u && TileDataOffsetPerLayer != null)
            {
    ...
    {
        if (!IsLegacyData() && TileOffsetData != null && level < TileOffsetData.Length)
            return TileOffsetData[level];

        var blockWidthInTiles = GetWidthInTiles();
        var blockHeightInTiles = GetHeightInTiles();
        var maxAddress = level + 1 < TileIndexPerMip.Length ? TileIndexPerMip[level + 1] : 0;
        return new FVirtualTextureTileOffsetData(blockWidthInTiles, blockHeightInTiles,
            Math.Max(maxAddress - TileIndexPerMip[level], 1));
    }
}
...
{
    public int NumNonOptionalMips { get; set; }
    public int NumNonStreamingMips { get; set; }
    public FVector LandscapeGridScale { get; set; }
    public FLandscapeTexture2DMipMap[] Mips { get; set; }
    public FPackageIndex Texture { get; set; }

    public ULandscapeTextureStorageProviderFactory()
    {
        Mips = Array.Empty<FLandscapeTexture2DMipMap>();
        Texture = new FPackageIndex();
        LandscapeGridScale = new FVector();
    }
    ...
    public void Deserialize(FAssetArchive Ar, long validPos)
    {
        ...
        Mips = Ar.ReadArray(() => new FLandscapeTexture2DMipMap(Ar)) ?? Array.Empty<FLandscapeTexture2DMipMap>();
    ...
    public unsafe void DecompressMip(byte[] sourceData, long sourceDataBytes, byte[] destData, long destDataBytes, int mipIndex)
    {
        ...
        if (sourceDataBytes != (totalPixels + borderPixels) * 2)
            throw new InvalidOperationException("源数据大小无效");
        if (destDataBytes != totalPixels * 4)
            throw new InvalidOperationException("目标数据大小无效");
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                else
                {
                    ...
                    for (int x = 1; x < width; x++)
                    {
                        ...
                        *topLeftPixel = new FColor(
                            topLeftPixel->R,
                            topLeftPixel->G,
                            (byte)Math.Clamp((TL_Normal.X * 127.5f + 127.5f), 0.0f, 255.0f),
                            (byte)Math.Clamp((TL_Normal.Y * 127.5f + 127.5f), 0.0f, 255.0f)
                        );

...
public class FNaniteResources
{
    ...
    {
        // Initialize all non-nullable fields with default values
        RootData = null!; // Using null-forgiving operator since we'll initialize it properly later
        StreamablePages = null!;
        ImposterAtlas = new ushort[0];
        HierarchyNodes = new FPackedHierarchyNode[0];
        HierarchyRootOffsets = new uint[0];
        PageStreamingStates = new FPageStreamingState[0];
        PageDependencies = new uint[0];
        AssemblyTransforms = new FMatrix3x4[0];
        MeshBounds = default!;

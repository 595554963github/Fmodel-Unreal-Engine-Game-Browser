...
public class UObject : AbstractPropertyHolder
{
    ...
    public virtual void Deserialize(FAssetArchive Ar, long validPos)
    {
        ...
        {
            if (Class == null)
                throw new ParserException(Ar, "找到未版本化的属性，但对象没有类");
    ...
    internal static void DeserializePropertiesUnversioned(List<FPropertyTag> properties, FAssetArchive Ar, UStruct struc)
    {
        ...
        {
            throw new ParserException(Ar, "缺少类型的道具映射" + type);
        }
        ...
        do
        {
            ...
            if (isNonZero)
            {
                if (propMappings.TryGetValue(val, out var propertyInfo))
                {
                    ...
                    {
                        throw new ParserException(Ar, $"{type}:序列化属性失败{propertyInfo.MappingType.Type} {propertyInfo.Name}.无法继续序列化(序列化{properties.Count}到目前为止的属性)");
                    }
                }
                else
                {
                    throw new ParserException(Ar, $"{type}:具有未知属性的值{val}.无法继续序列化(序列化{properties.Count}到目前为止的属性)");
                }
            }
...
public static class PropertyUtil
{
    ...
    public static T Get<T>(IPropertyHolder holder, string name, StringComparison comparisonType = StringComparison.Ordinal)
    {
        ...
        {
            throw new NullReferenceException($"{holder.GetType().Name}没有属性'{name}'");
        }
        ...
        throw new NullReferenceException($"无法获取'{name}'类型的属性{typeof(T).Name} in {holder.GetType().Name}");
    }
    ...
    public static T GetByIndex<T>(IPropertyHolder holder, int index)
    {
        ...
        if (tag == null)
        {
            throw new NullReferenceException($"{holder.GetType().Name}在索引处没有属性'{index}'");
        }
        ...
        throw new NullReferenceException($"无法获得类型的属性{typeof(T).Name}在索引'{index}' in {holder.GetType().Name}");
    }
    ...
    public static void Set<T>(IPropertyHolder holder, string name, T value, StringComparison comparisonType = StringComparison.Ordinal)
    {
        ...
        tag2.Tag = value switch
        {
            ...
            _ => throw new NotImplementedException($"类型的设置属性{typeof(T).Name}尚未实施")
        };

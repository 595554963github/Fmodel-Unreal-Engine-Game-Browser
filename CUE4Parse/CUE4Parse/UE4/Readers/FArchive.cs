...
namespace CUE4Parse.UE4.Readers
{
    public abstract class FArchive : RandomAccessStream, ICloneable
    {
        ...
        public virtual byte[] ReadBytes(int length)
        {
            ...
            {
                int read = Read(result, totalRead, length - totalRead);
                if (read == 0)
                {
                    throw new ParserException(this, $"读取字节失败,请求长度:{length},实际读取:{totalRead}");
                }
                totalRead += read;
            }
            return result;
        ...
        public T[] ReadBulkArray<T>(int elementSize, int elementCount, Func<T> getter)
        {
            ...
            if (Game != EGame.GAME_霍格沃茨遗产 && Position != pos + array.Length * elementSize)
        ...
        public bool ReadBoolean()
        {
            ...
            return i switch
            {
                0 => false,
                1 => true,
                _ => throw new ParserException(this, $"布尔值无效({i})")
            };
        }
        ...
        public int Read7BitEncodedInt()
        {
            ...
            do
            {
                if (shift == 5 * 7)  // 5 bytes max per Int32, shift += 7
                    throw new FormatException("流已损坏");
        ...
        {
            throw new InvalidOperationException("通用FArchive无法读取UObject的");
        }
        ...
        public void SerializeCompressedNew(byte[] dest, int length, string compressionFormatToDecodeOldV1Files, ECompressionFlags flags, bool bTreatBufferAsFileReader, out long outPartialReadLength)
        {
            ...
            if (!bHeaderWasValid)
            {
                throw new ParserException(this, "压缩数据头验证失败,包文件可能已损坏!");
            }
            ...
            {
                throw new ParserException(this, "BulkData压缩标头读取错误,此包可能已损坏!\n未找到解码的压缩格式:" + compressionFormatToDecode);
            }
            ...
            if (summary.UncompressedSize > length) throw new ParserException(this, $"档案序列化压缩未压缩大小({summary.UncompressedSize}) > 长度({length})");
            ...
            if (totalChunkCompressedSize != summary.CompressedSize) throw new ParserException(this, $"档案序列化压缩总块压缩大小({totalChunkCompressedSize}) != 摘要.压缩大小({summary.CompressedSize})");
            if (totalChunkUncompressedSize != summary.UncompressedSize) throw new ParserException(this, $"档案序列化压缩的总块未压缩大小({totalChunkUncompressedSize}) != 摘要.未压缩大小({summary.UncompressedSize})");
            ...
            for (var chunkIndex = 0; chunkIndex < totalChunkCount; chunkIndex++)
            {
                ...
                int totalRead = 0;
                while (totalRead < chunk.CompressedSize)
                {
                    int read = Read(compressedBuffer, totalRead, (int)chunk.CompressedSize - totalRead);
                    if (read == 0)
                    {
                        throw new ParserException(this, $"读取压缩块失败,请求长度:{chunk.CompressedSize},实际读取:{totalRead}");
                    }
                    totalRead += read;
                }

...
public class ULevel : Assets.Exports.UObject
{
    ...
    public override void Deserialize(FAssetArchive Ar, long validPos)
    {
        ...
        // 确保所有字段在使用前都被初始化
        Actors = Ar.ReadArray(() => new FPackageIndex(Ar)) ?? Array.Empty<FPackageIndex>();
        URL = new FURL(Ar);
        Model = new FPackageIndex(Ar);
        ModelComponents = Ar.ReadArray(() => new FPackageIndex(Ar)) ?? Array.Empty<FPackageIndex>();
        ...
        if (Ar.Game == EGame.GAME_地铁觉醒 && GetOrDefault<bool>("bIsLightingScenario")) return;
        if (Ar.Game == EGame.GAME_腐烂国度2 && Ar.ReadBoolean()) return;
    ...
    protected internal override void WriteJson(JsonWriter writer, JsonSerializer serializer)
    {
        ...
        serializer.Serialize(writer, Actors ?? Array.Empty<FPackageIndex>());

        writer.WritePropertyName("URL");
        serializer.Serialize(writer, URL); // Just serialize the existing URL or null

        writer.WritePropertyName("Model");
        serializer.Serialize(writer, Model);

        writer.WritePropertyName("ModelComponents");
        serializer.Serialize(writer, ModelComponents ?? Array.Empty<FPackageIndex>());

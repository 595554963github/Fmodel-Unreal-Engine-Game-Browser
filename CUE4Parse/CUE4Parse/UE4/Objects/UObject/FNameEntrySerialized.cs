...
namespace CUE4Parse.UE4.Objects.UObject
{
    public readonly struct FNameEntrySerialized
    {
        ...
        {
            var bHasNameHashes = Ar.Ver >= EUnrealEngineObjectUE4Version.NAME_HASHES_SERIALIZED || Ar.Game is EGame.GAME_战争机器4 or EGame.GAME_往日不再;

            Name = Ar.ReadFString().Trim();

            if (Ar.Game == EGame.GAME_绝地求生大逃杀)
            {
                if (_pubgNameMap == null)
                {
                    ...
                    if (stream == null) throw new MissingManifestResourceException("在嵌入式资源中找不到PUBGNameHashMap.json");

...
{
    private Dictionary<FPackageIndex, Dictionary<FName, string>> ObjectMetaDataMap = new();
    private Dictionary<FName, string> RootMetaDataMap = new();

    public override void Deserialize(FAssetArchive Ar, long validPos)
    {
        base.Deserialize(Ar, validPos);

        ObjectMetaDataMap = Ar.ReadMap(() => (new FPackageIndex(Ar), Ar.ReadMap(() => (Ar.ReadFName(), Ar.ReadFString() ?? string.Empty))));

        if (FEditorObjectVersion.Get(Ar) >= FEditorObjectVersion.Type.RootMetaDataSupport)
        {
            RootMetaDataMap = Ar.ReadMap(() => (Ar.ReadFName(), Ar.ReadFString() ?? string.Empty));
        }
    }

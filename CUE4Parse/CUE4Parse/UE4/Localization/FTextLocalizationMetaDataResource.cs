...
namespace CUE4Parse.UE4.Localization
{
    ...
    public class FTextLocalizationMetaDataResource
    {
        ...
        public FTextLocalizationMetaDataResource(FArchive Ar)
        {
            ...
            {
                Ar.Position = 0;
                Log.Warning($"本地化元数据文件'{Ar.Name}'魔数校验失败!");
            }

            // 此本地化资源文件版本是否高于当前加载器支持的版本?
            if (versionNumber > ELocMetaVersion.Latest)
            {
                throw new ParserException(Ar, $"本地化元数据文件'{Ar.Name}'版本过高，无法加载(文件版本:{versionNumber:D},加载器版本:{ELocMetaVersion.Latest:D})");
            }

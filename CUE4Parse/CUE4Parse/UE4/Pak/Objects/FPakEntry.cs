...
public class FPakEntry : VfsEntry
{
    ...
    public FPakEntry(PakFileReader reader, string path, FArchive Ar) : base(reader, path)
    {
        ...
        if (Ar.Game == GAME_铁拳7) Flags = (uint)(Flags & ~Flag_Encrypted);
    ...
    endRead:
        ...
        if (Ar.Game == GAME_腐烂国度2 && CompressionMethod == CompressionMethod.None) StructSize = 0;
    }
    ...
    public unsafe FPakEntry(PakFileReader reader, string path, byte* data) : base(reader, path)
    {
        ...
        if (reader.Game == GAME_鸣潮 && reader.Info.Version > PakFile_Version_Fnv64BugFix)
        {
        ...
        if (reader.Ar.Game == GAME_尘白禁区) Offset ^= 0x1F1E1D1C;
        if (reader.Ar.Game is GAME_QQ_这tm算游戏 or GAME_元梦之星) Offset += 8;
    ...
    public FPakEntry(PakFileReader reader, string path, FArchive Ar, int offset) : base(reader, path)
    {
        ...
        if (reader.Game == GAME_尘白禁区) Offset ^= 0x1F1E1D1C;
        if (reader.Game is GAME_QQ_这tm算游戏 or GAME_元梦之星) Offset += 8;

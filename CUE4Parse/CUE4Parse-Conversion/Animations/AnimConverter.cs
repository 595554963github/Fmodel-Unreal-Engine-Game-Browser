...
namespace CUE4Parse_Conversion.Animations
{
    public static class AnimConverter
    {
        ...
        public static CAnimSet ConvertAnims(this USkeleton skeleton, UAnimComposite? animComposite)
        {
            ...
            if (animComposite.AnimationTrack == null)
            {
                return animSet;
            }
            ...
            {
                if (segment.AnimReference.TryLoad(out UObject? obj) && obj is UAnimSequence animSequence)
                {
        ...
        public static CAnimSet ConvertAnims(this USkeleton skeleton, UAnimMontage? animMontage)
        {
            ...
            {
                if (slotAnimTrack.AnimTrack == null)
                    continue;
            ...
            if (animMontage.CompositeSections != null && animMontage.CompositeSections.Length > 0)
            {
                var compositeSection = animMontage.CompositeSections[0];
                do
                {
                    if (compositeSection.LinkedSequence.TryLoad(out UObject? obj) && obj is UAnimSequence animSequence)
                    {
                        if (compositeSection.SlotIndex < animMontage.SlotAnimTracks.Length &&
                            compositeSection.SegmentIndex < animMontage.SlotAnimTracks[compositeSection.SlotIndex].AnimTrack.AnimSegments.Length)
                        {
                ...
                } while (compositeSection != null && !compositeSection.NextSectionName.IsNone &&
                         compositeSection.SectionName != compositeSection.NextSectionName);
            }

            if (animMontage.CompositeSections != null)
            {
                foreach (var compositeSection in animMontage.CompositeSections)
                {
                        ...
                        compositeSection.SegmentIndex < animMontage.SlotAnimTracks[compositeSection.SlotIndex].AnimTrack.AnimSegments.Length)
                    {
                        ...
                        if (segment.AnimReference.TryLoad(out UObject? obj1) && obj1 is UAnimSequence loadedAnim1)
                        {
                            animSequence = loadedAnim1;
                        }
                        else if (compositeSection.LinkedSequence.TryLoad(out UObject? obj2) && obj2 is UAnimSequence loadedAnim2)
                        {
                            animSequence = loadedAnim2;
                        }
        ...
        private static void ReadPerTrackQuatData(FArchive Ar, string trackKind, ref FQuat[] dstKeys, ref float[] dstTimeKeys, int numFrames)
        {
            ...
            for (var keyIndex = 0; keyIndex < numKeys; keyIndex++)
            {
                dstKeys[keyIndex] = keyFormat switch
                {
                    ...
                    _ => throw new ParserException(Ar, $"未知{trackKind}压缩方法:{(int)keyFormat} ({keyFormat})")
                };
            }
            // align to 4 bytes
            Ar.Position = Ar.Position.Align(4);
            ...
            for (var keyIndex = 0; keyIndex < numKeys; keyIndex++)
            {
                switch (keyFormat)
                {
                    ...
                    case ACF_Identity:
                        ...
                        break;
                    default:
                        throw new ParserException(Ar, $"未知{trackKind}压缩方法:{(int)keyFormat} ({keyFormat})");
                }
            }
            Ar.Position = Ar.Position.Align(4);
            if (hasTimeTracks)
        ...
        {
            if (animSequence.CompressedDataStructure == null)
            {
                throw new InvalidOperationException("CompressedDataStructure在动画序列中为空");
            }
            if (!(animSequence.CompressedDataStructure is FUECompressedAnimData compressedData))
            {
                throw new InvalidOperationException("CompressedDataStructure不是FUECompressedAnimData");
            }
            ...
            if (compressedData.CompressedTrackOffsets == null || trackIndex * 2 + 1 >= compressedData.CompressedTrackOffsets.Length)
            {
                throw new IndexOutOfRangeException("无效的trackIndex或CompressedTrackOffsets数组");
            }
            ...
            {
                if (compressedData.CompressedScaleOffsets.OffsetData == null || trackIndex >= compressedData.CompressedScaleOffsets.OffsetData.Length)
                {
                    throw new IndexOutOfRangeException("无效的trackIndex或OffsetData数组");
                }
                scaleOffset = compressedData.CompressedScaleOffsets.OffsetData[trackIndex];
            }
            if (transOffset == -1)
            {
        ...
        private static void ReadKeyLerpData(FArchive reader, UAnimSequence animSequence, CAnimTrack track, int trackIndex, bool hasTimeTracks)
        {
            ...
            if (compressedData.CompressedTrackOffsets == null || trackIndex * 4 + 3 >= compressedData.CompressedTrackOffsets.Length)
            {
                throw new IndexOutOfRangeException($"无效的trackIndex {trackIndex}或者CompressedTrackOffsets array");
            }
            ...
            if (compressedData.CompressedScaleOffsets.IsValid())
            {
                if (compressedData.CompressedScaleOffsets.OffsetData == null || trackIndex * 2 + 1 >= compressedData.CompressedScaleOffsets.OffsetData.Length)
                {
                    throw new IndexOutOfRangeException($"Invalid trackIndex {trackIndex} or OffsetData array");
                }
            ...
            track.KeyPos = transKeys > 0 ? new FVector[transKeys] : Array.Empty<FVector>();
            track.KeyQuat = rotKeys > 0 ? new FQuat[rotKeys] : Array.Empty<FQuat>();
            track.KeyScale = scaleKeys > 0 ? new FVector[scaleKeys] : Array.Empty<FVector>();
            void ReadVectorKeys(long offset, int numKeys, AnimationCompressionFormat compressionFormat, FVector[] keys)
            {
                if (numKeys <= 0 || keys.Length < numKeys) return;

                reader.Position = offset;
                if (numKeys == 1) compressionFormat = ACF_None;

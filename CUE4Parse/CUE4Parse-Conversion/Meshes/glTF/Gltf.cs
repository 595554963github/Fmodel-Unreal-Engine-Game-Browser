...
namespace CUE4Parse_Conversion.Meshes.glTF
{
    ...
    public class Gltf
    {
        ...
        public Gltf(string name, CSkelMeshLod lod, List<CSkelMeshBone> bones, List<MaterialExporter2>? materialExports, ExporterOptions options, FPackageIndex[]? morphTargets = null, int lodIndex = -1)
        {
            ...
            {
                var targetNames = new JsonObject();
                var targetNamesArray = new JsonArray();
                targetNames["targetNames"] = targetNamesArray;

                for (var i = 0; i < morphTargets.Length; i++)
                {
                    var morphTarget = morphTargets[i]?.Load<UMorphTarget>();
                    if (morphTarget == null ||
                        morphTarget.MorphLODModels == null ||
                        morphTarget.MorphLODModels.Length <= lodIndex ||
                        lodIndex < 0)
                    {
                        continue;
                    }
                    ...
                    if (morphTarget.Name != null)
                    {
                        targetNamesArray.Add(morphTarget.Name);
                    }

                    if (morphModel.Vertices == null || lod.Verts == null)
                    {
                        continue;
                    }
                    ...
                    for (int j = 0; j < morphModel.Vertices.Length; j++)
                    {
                        var delta = morphModel.Vertices[j];
                        if (delta.SourceIdx < 0 || delta.SourceIdx >= lod.Verts.Length)
                        {
                            continue;
                        }

                        var vert = lod.Verts[delta.SourceIdx];
                        var srcVert = new VertexPositionNormalTangent(
                            SwapYZ(vert.Position * 0.01f),
                            SwapYZAndNormalize((FVector)vert.Normal),
                            SwapYZAndNormalize((Vector4)vert.Tangent));
                        ...
                        // Get the vertex directly from the array we already have
                        var vertex = verts[index];
                        morphBuilder.SetVertexDelta(
                            vertex,
                            new VertexGeometryDelta(
                                SwapYZ(delta.PositionDelta * 0.01f),
                                Vector3.Zero,

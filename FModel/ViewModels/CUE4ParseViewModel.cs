...
public class CUE4ParseViewModel : ViewModel
{
    ...
    public async Task Initialize()
    {
        await _threadWorkerView.Begin(cancellationToken =>
        {
                    ...
                    switch (p.LiveGame)
                    {
                        case "FortniteLive":
                        {
                            ...
                            FLogger.Append(ELog.Information, () =>
                                FLogger.Text($"堡垒之夜[LIVE]已成功加载{elapsedTime.TotalMilliseconds:F1}ms", Constants.WHITE, true));
                            break;
                        }
                        case "ValorantLive":
                        {
            ...
            Provider.Initialize();
            Log.Information($"{Provider.Versions.Game} ({Provider.Versions.Platform}) | 档案: x{Provider.UnloadedVfs.Count} | AES: x{Provider.RequiredKeys.Count} | 松散文件: x{Provider.Files.Count}");
        });
    }
    ...
    public void LoadVfs(IEnumerable<KeyValuePair<FGuid, FAesKey>> aesKeys)
    {
        ...
        Log.Information($"对象: {Provider.ProjectName} | 挂载: {Provider.MountedVfs.Count}/{archiveMax} | AES: {Provider.Keys.Count}/{aesMax} | 文件: x{Provider.Files.Count}");
    }
    ...
    public Task VerifyConsoleVariables()
    {
        ...
        {
            FLogger.Append(ELog.Warning, () =>
                FLogger.Text($"选择UE测试版本可能不适合'{Provider.GameDisplayName ?? Provider.ProjectName}'", Constants.WHITE, true));
        }
    ...
    public async Task LoadLocalizedResources()
    {
        ...
        {
            FLogger.Append(ELog.Information, () =>
                FLogger.Text($"{LocalizedResourcesCount}加载的本地化资源'{UserSettings.Default.AssetLanguage.GetDescription()}'", Constants.WHITE, true));
    ...
    public Task LoadVirtualPaths()
    {
        ...
        return Task.Run(() =>
        {
            ...
            {
                FLogger.Append(ELog.Information, () =>
                    FLogger.Text($"已加载{_virtualPathCount}条虚拟路径", Constants.WHITE, true));
            }
            else
            {
                FLogger.Append(ELog.Warning, () =>
    ...
    {
        Log.Information("用户双击以提取'{FullPath}'", entry.Path);
        ...
        switch (entry.Extension)
        {
            ...
            case "ttf":
                FLogger.Append(ELog.Warning, () =>
                    FLogger.Text($"导出'{entry.Name}'原始数据并更改其扩展名，如果您希望它是可安装的字体文件", Constants.WHITE, true));
                break;
            ...
            {
                FLogger.Append(ELog.Warning, () =>
                    FLogger.Text($"包'{entry.Name}'是未知类型.", Constants.WHITE, true));
                break;
            }
        }
    }
    ...
    {
        Log.Information("用户双击以提取'{FullPath}'", fullPath);
            ...
            case USvgAsset when (isNone || saveTextures) && pointer.Object.Value is USvgAsset svgasset:
                {
                    ...
                    if (saveTextures)
                    {
                        ...
                        if (File.Exists(path))
                        {
                            Log.Information("{FileName}成功提取", fileName);
                            if (updateUi)
                            {
                                FLogger.Append(ELog.Information, () =>
                                {
                                    FLogger.Text("成功提取", Constants.WHITE);
                        ...
                        {
                            Log.Error("{FileName}无法保存", fileName);
                            if (updateUi)
    ...
    private void SaveAndPlaySound(string fullPath, string ext, byte[] data)
    {
        ...
        {
            var audioPlayer = Helper.GetWindow<AudioPlayer>("音频播放器", () => new AudioPlayer().Show());
    ...
    private void SaveExport(UObject export, bool updateUi = true)
    {
        ...
        {
            Log.Information("成功保存{FilePath}", savedFilePath);
            if (updateUi)
            {
                FLogger.Append(ELog.Information, () =>
                {
        ...
        {
            Log.Error("{FileName}无法保存", export.Name);
    ...
    public void ExportData(GameFile entry, bool updateUi = true)
    {
        if (Provider.TrySavePackage(entry, out var assets))
        {
            ...
            Log.Information("{FileName}成功导出", entry.Name);
            if (updateUi)
            {
                FLogger.Append(ELog.Information, () =>
                {
        ...
        {
            Log.Error("{FileName}无法导出", entry.Name);
            if (updateUi)

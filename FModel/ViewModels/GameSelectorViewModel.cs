...
public class GameSelectorViewModel : ViewModel
{
    ...
    private IEnumerable<DirectorySettings> EnumerateDetectedGames()
    {
        ...
        yield return DirectorySettings.Default("堡垒之夜[LIVE]", Constants._FN_LIVE_TRIGGER, ue: EGame.GAME_UE5_6);
        yield return GetUnrealEngineGame("Pewee", "\\RogueCompany\\Content\\Paks", EGame.GAME_侠盗公司);
        yield return GetUnrealEngineGame("Rosemallow", "\\Indiana\\Content\\Paks", EGame.GAME_UE4_21);
        yield return GetUnrealEngineGame("Catnip", "\\OakGame\\Content\\Paks", EGame.GAME_无主之地3);
        yield return GetUnrealEngineGame("AzaleaAlpha", "\\Prospect\\Content\\Paks", EGame.GAME_UE4_27);
        yield return GetUnrealEngineGame("shoebill", "\\SwGame\\Content\\Paks", EGame.GAME_星球大战绝地_陨落的武士团);
        yield return GetUnrealEngineGame("Snoek", "\\StateOfDecay2\\Content\\Paks", EGame.GAME_腐烂国度2);
        yield return GetUnrealEngineGame("711c5e95dc094ca58e5f16bd48e751d6", "\\MultiVersus\\Content\\Paks", EGame.GAME_UE4_26);
        yield return GetUnrealEngineGame("9361c8c6d2f34b42b5f2f61093eedf48", "\\TslGame\\Content\\Paks", EGame.GAME_绝地求生大逃杀);
        yield return GetRiotGame("VALORANT", "ShooterGame\\Content\\Paks", EGame.GAME_无畏契约);
        yield return DirectorySettings.Default("无畏契约[LIVE]", Constants._VAL_LIVE_TRIGGER, ue: EGame.GAME_无畏契约);
        yield return GetSteamGame(381210, "\\DeadByDaylight\\Content\\Paks", EGame.GAME_UE4_27); // Dead By Daylight
        yield return GetSteamGame(578080, "\\TslGame\\Content\\Paks", EGame.GAME_绝地求生大逃杀); // PUBG
        yield return GetSteamGame(1172380, "\\SwGame\\Content\\Paks", EGame.GAME_星球大战绝地_陨落的武士团); // STAR WARS Jedi: Fallen Order™
        yield return GetSteamGame(677620, "\\PortalWars\\Content\\Paks", EGame.GAME_分裂之门); // Splitgate
        yield return GetSteamGame(1172620, "\\Athena\\Content\\Paks", EGame.GAME_盗贼之海); // Sea of Thieves
        yield return GetSteamGame(1665460, "\\pak", EGame.GAME_UE4_26); // eFootball 2023
        yield return GetRockstarGamesGame("GTA III - Definitive Edition", "\\Gameface\\Content\\Paks", EGame.GAME_侠盗猎车手_三部曲_终级版);
        yield return GetRockstarGamesGame("GTA San Andreas - Definitive Edition", "\\Gameface\\Content\\Paks", EGame.GAME_侠盗猎车手_三部曲_终级版);
        yield return GetRockstarGamesGame("GTA Vice City - Definitive Edition", "\\Gameface\\Content\\Paks", EGame.GAME_侠盗猎车手_三部曲_终级版);
        yield return GetLevelInfiniteGame("tof_launcher", "\\Hotta\\Content\\Paks", EGame.GAME_幻塔);
    }
    ...
    private DirectorySettings GetUnrealEngineGame(string gameName, string pakDirectory, EGame ueVersion)
    {
        ...
        if (_launcherInstalled?.InstallationList != null)
        {
            foreach (var installationList in _launcherInstalled.InstallationList)
            {
                ...
                {
                    Log.Debug("在LauncherInstale.dat中找到{GameName}", gameName);
    ...
    private DirectorySettings GetRiotGame(string gameName, string pakDirectory, EGame ueVersion)
    {
        ...
        if (_riotClientInstalls is { AssociatedClient: { } })
        {
            foreach (var (key, _) in _riotClientInstalls.AssociatedClient)
            {
                ...
                {
                    Log.Debug("在RiotClientInstalls.json中找到{GameName}", gameName);
    ...
    private DirectorySettings GetSteamGame(int id, string pakDirectory, EGame ueVersion)
    {
        ...
        {
            Log.Debug("在steam manifests找到{GameName}", steamInfo.Name);
    ...
    private DirectorySettings GetRockstarGamesGame(string key, string pakDirectory, EGame ueVersion)
    {
        ...
        {
            Log.Debug("在注册表中找到{GameName}", key);
    ...
    private T GetDriveLauncherInstalls<T>(string jsonFile)
    {
        foreach (var drive in DriveInfo.GetDrives())
        {
            ...
            Log.Debug("\"{Launcher}\"在驱动器中找到\"{DriveName}\"", launcher, drive.Name);

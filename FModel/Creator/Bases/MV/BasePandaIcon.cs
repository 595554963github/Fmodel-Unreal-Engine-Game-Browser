...
public class BasePandaIcon : UCreator
{
    ...
    private new void DrawBackground(SKCanvas c)
    {
        c.DrawRect(new SKRect(0, 0, Width, Height),
            ...
            {
                IsAntialias = true,
                Color = SKColor.Parse("#F3FCF0")
            });

        var has_tr = _rarity != ERewardRarity.None;
        var tr = Utils.GetLocalizedResource(_rarity);
        var trFont = new SKFont(Utils.Typefaces.DisplayName, 35);
        var trPaint = new SKPaint
        {
            IsAntialias = true,
        ...
        {
            const int margin = 15;
            var width = trFont.MeasureText(tr);
        ...
        if (has_tr)
        {
            ...
            {
                c.DrawText(a, x, 40, SKTextAlign.Right, trFont, trPaint);
                x -= trFont.MeasureText(a) - 2;
            }
        }
    }
    ...
    private new void DrawPreview(SKCanvas c)
    {
        ...
        var source = Preview ?? DefaultPreview;
        if (source != null)
        {
            using var resized = source.Resize(new SKImageInfo(size, size), new SKSamplingOptions(SKCubicResampler.Mitchell));
            c.DrawBitmap(resized, new SKRect(Margin, y, size + Margin, y + size));
        }
    }
    ...
    private new void DrawDisplayName(SKCanvas c)
    {
        ...
        var font = _displayNameFont;
        var x = 450f;
        while (font.MeasureText(DisplayName) > Width - x / 1.25)
        {
            font.Size -= 1;
        }

        var y = Height / 2 - font.Size / 4;
        foreach (var a in DisplayName.Select(character => character.ToString()))
        {
            c.DrawText(a, x, y, SKTextAlign.Left, font, DisplayNamePaint);
            x += font.MeasureText(a) - 4;
        }
    }
    ...
    private new void DrawDescription(SKCanvas c)
    {
        ...
        // 传递 SKTextAlign.Left 到 DrawText
        c.DrawText(_type.ToUpper(), x, 170, SKTextAlign.Left, _descriptionFont, DescriptionPaint);

        if (string.IsNullOrWhiteSpace(Description)) return;

        DescriptionPaint.Color = SKColor.Parse("#191C33");
        // 参考后者的多行文本绘制逻辑（直接实现，不依赖外部方法）
        var lines = Description.Split('\n');
        float currentY = _y_offset;
        foreach (var line in lines)
        {
            if (string.IsNullOrEmpty(line)) continue;

            var textWidth = _descriptionFont.MeasureText(line);
            if (textWidth > Width - x)
            {
                ...
                foreach (var word in words)
                {
                    ...
                    var testWidth = _descriptionFont.MeasureText(testLine);
                    if (testWidth <= Width - x)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            c.DrawText(currentLine, x, currentY, SKTextAlign.Left, _descriptionFont, DescriptionPaint);
                            currentY += _descriptionFont.Spacing + 2;
                        }
                        currentLine = word;
                    }
                }
            ...
            {
                c.DrawText(line, x, currentY, SKTextAlign.Left, _descriptionFont, DescriptionPaint);
                currentY += _descriptionFont.Spacing + 2;
            }
        }
        _y_offset = currentY;
    ...
    private void DrawPictos(SKCanvas c)
    {
        ...
        var font = new SKFont(Utils.Typefaces.Default, 27);
        ...
        foreach (var picto in Pictos)
        {
            if (picto.Item1 != null)
            {
                using var resized = picto.Item1.Resize(new SKImageInfo(size, size), new SKSamplingOptions(SKCubicResampler.Mitchell));
                c.DrawBitmap(resized, new SKRect(x, _y_offset + 10, x + size, _y_offset + 10 + size), ImagePaint);
            }
            c.DrawText(picto.Item2, x + size + 10, _y_offset + size + 6, SKTextAlign.Left, font, paint);
            _y_offset += size + 5;
        }
    }
}

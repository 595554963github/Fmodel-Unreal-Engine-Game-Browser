...
public class BaseTandem : BaseIcon
{
    ...
    private readonly SKPaint _panelPaint = new()
    {
        ...
        Color = SKColor.Parse("#0045C7")
    };

    private new void DrawBackground(SKCanvas c)
    {
        c.DrawBitmap(SKBitmap.Decode(Application.GetResourceStream(new Uri("pack://application:,,,/Resources/npcleftside.png"))?.Stream).Resize(Width, Height),
            0, 0, new SKPaint
            {
                IsAntialias = false,
                ImageFilter = SKImageFilter.CreateBlur(0, 25),
            });
    ...
    private void DrawHaze(SKCanvas c)
    {
        ...
        _panelPaint.Shader = SKShader.CreateLinearGradient(new SKPoint(343, 0), new SKPoint(343, Height),
            new[] { SKColors.Transparent, SKColor.Parse("#001E70FF"), SKColor.Parse("#001E70").WithAlpha(200), SKColor.Parse("#001E70").WithAlpha(245), SKColor.Parse("#001E70") },
            new[] { 0, (float).1, (float).65, (float).85, 1 }, SKShaderTileMode.Clamp);
        c.DrawPath(rect1, _panelPaint);
    }

    private readonly SKFont _displayNameFont = new(Utils.Typefaces.TandemDisplayName, 42);
    private readonly SKFont _descriptionFont = new(Utils.Typefaces.TandemGenDescription, 17);

    private void DrawName(SKCanvas c)
    {
        if (string.IsNullOrWhiteSpace(DisplayName)) return;

        _displayNameFont.Size = UserSettings.Default.AssetLanguage switch
        {
            ELanguage.Korean => 56,
            _ => 42
        };

        _displayNameFont.ScaleX = (float)1.1;
        _displayNameFont.SkewX = (float)-.25;
        ...
        {
            _displayNameFont.Size = 30;
        }

        _displayNameFont.Typeface = typeface;
    ...
    private void DrawGeneralDescription(SKCanvas c)
    {
        ...
        _descriptionFont.Size = UserSettings.Default.AssetLanguage switch
        {
            ELanguage.Korean => 20,
            _ => 17
        };
        ...
        if (typeface == Utils.Typefaces.Default)
        {
            _descriptionFont.Size = 21;
        }

        _descriptionFont.Typeface = typeface;

        c.DrawText(
            _generalDescription.ToUpper(),
            97,
            930,
            _descriptionFont,
            DescriptionPaint
    ...
    private void DrawAdditionalDescription(SKCanvas c)
    {
        ...
        Utils.DrawMultilineText(
            ...
            97,
            SKTextAlign.Left,
            new SKRect(97, 960, Width - 10, Height),
            _descriptionFont, 
            DescriptionPaint, 
            out var _       
        );

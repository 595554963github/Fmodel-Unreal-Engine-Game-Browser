...
public class BaseCommunity : BaseIcon
{
    ...
    public override void ParseForInfo()
    {
        ...
        else if (Object.TryGetValue(out FInstancedStruct[] dataList, "DataList") && dataList.FirstOrDefault(d => d.NonConstStruct?.TryGetValue(out FPackageIndex _, "Series") == true) is { } dl)
            _rarityName = dl.NonConstStruct?.Get<FPackageIndex>("Series").Name;
        else if (Object.TryGetValue(out FStructFallback componentContainer, "ComponentContainer") && componentContainer.TryGetValue(out FPackageIndex[] components, "Components") && components.FirstOrDefault(c => c.Name.Contains("Series")) is { } seriesDef && seriesDef.TryLoad(out var seriesDefObj) && seriesDefObj is not null && seriesDefObj.TryGetValue(out series, "Series"))
            _rarityName = series.Name;
        else
    ...
    private string GetCosmeticSet(string setName, bool bShort)
        => bShort ? setName["Cosmetics.Set.".Length..] : base.GetCosmeticSet(setName);
    ...
    private new void DrawDisplayName(SKCanvas c)
    {
        ...
        {
            using var displayNameFont = new SKFont(SKTypeface.Default, font.FontSize)
            {
                Edging = SKFontEdging.SubpixelAntialias,
                ScaleX = font.FontScale,
                SkewX = font.SkewValue
            };
            ...
            if (font.Typeface.TryGetValue(UserSettings.Default.AssetLanguage, out var path) || font.Typeface.TryGetValue(ELanguage.English, out path))
                typeface = Utils.Typefaces.OnTheFly(path);

            displayNameFont.Typeface = typeface;

            while (displayNameFont.MeasureText(DisplayName) > Width - Margin * 2)
                displayNameFont.Size -= 1;

            var x = font.Alignment switch
            {
                SKTextAlign.Center => Width / 2f,
                SKTextAlign.Right => Width - Margin,
                _ => Margin
            };

            c.DrawText(DisplayName, x, font.Y, displayNameFont, DisplayNamePaint);
        }
        else
    ...
    private new void DrawDescription(SKCanvas c)
    {
        ...
        {
            using var descriptionFont = new SKFont(SKTypeface.Default, font.FontSize)
            {
                Edging = SKFontEdging.SubpixelAntialias,
                ScaleX = font.FontScale,
                SkewX = font.SkewValue
            ...
            descriptionFont.Typeface = typeface;

            while (descriptionFont.MeasureText(Description) > Width - Margin * 2)
                descriptionFont.Size -= 1;
            ...
            if (font.MaxLineCount < 2)
            {
                ...
                c.DrawText(Description, x, font.Y, descriptionFont, DescriptionPaint);
            }
            else
            {
                Utils.DrawMultilineText(
                    ...
                    Margin,
                    font.Alignment,
                    new SKRect(Margin, font.Y, Width - Margin, Height),
                    descriptionFont,  
                    DescriptionPaint, 
                    out var _ 
                );
    ...
    private void DrawToBottom(SKCanvas c, FontDesign font, string text)
    {
        ...
        while (displayNameFont.MeasureText(text) > Width - Margin * 2)
            displayNameFont.Size -= 1;
        ...
        c.DrawText(text, x, font.Y, displayNameFont, DisplayNamePaint);
    }

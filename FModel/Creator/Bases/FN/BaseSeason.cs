...
public class BaseSeason : UCreator
{
    ...
    private readonly SKFont _headerFont = new(Utils.Typefaces.Bundle, 50);
    private readonly SKPaint _headerPaint = new()
    {
        IsAntialias = true,
        Color = SKColor.Parse("#262630"),
        ImageFilter = SKImageFilter.CreateMatrix(
    ...
    public void DrawHeader(SKCanvas c)
    {
        ...
        _headerPaint.Shader = SKShader.CreateRadialGradient(
            new SKPoint(Width / 2, _headerHeight / 2),
            Width / 5 * 4,
            new[] { SKColors.SkyBlue.WithAlpha(50), SKColors.Blue.WithAlpha(50) },
            SKShaderTileMode.Clamp
        );
        c.DrawRect(new SKRect(0, 0, Width, Height), _headerPaint);

        _headerPaint.Shader = SKShader.CreateLinearGradient(
            new SKPoint(Width / 2, _headerHeight),
            new SKPoint(Width / 2, 75),
            new[] { SKColors.Black.WithAlpha(25), SKColors.Blue.WithAlpha(0) },
            SKShaderTileMode.Clamp
        );
        ...
        while (_headerFont.MeasureText(DisplayName) > Width)
        {
            _headerFont.Size -= 1;
        }
        ...
        c.DrawText(
            DisplayName,
            Width / 2f,
            _headerHeight / 2f + _headerFont.Size / 2 - 10,
            _headerFont,
            _headerPaint
        );
    }

...
public class BaseIconStats : BaseIcon
{
    ...
    public override void ParseForInfo()
    {
        ...
        {
            var tempFont = new SKFont(_informationFont.Typeface, _informationFont.Size);
            ...
            if (!string.IsNullOrEmpty(Description))
            {
                var font = new SKFont(_informationFont.Typeface, _informationFont.Size);

                Height += 40 + (int)font.Size * Utils.SplitLines(Description, font, Width - 20).Count;
            }
        }
    }
    ...
    {
        IsAntialias = true,
        ImageFilter = SKImageFilter.CreateMatrix(SKMatrix.CreateScale(1, 1), new SKSamplingOptions(SKFilterMode.Nearest, SKMipmapMode.None)),
        Color = new SKColor(0x26, 0x26, 0x30, 0xFF)
    };

    private readonly SKFont _informationFont = new(SKTypeface.Default, 16);
    private readonly SKFont _displayNameFont = new(SKTypeface.Default, 50);
    private readonly SKFont _statNameFont = new(SKTypeface.Default, 25);
    private readonly SKFont _statValueFont = new(SKTypeface.Default, 25);
    ...
    private new void DrawDisplayName(SKCanvas c)
    {
        ...
        _displayNameFont.Size = 50;
        _displayNameFont.Typeface = Utils.Typefaces.Bundle;
        _informationPaint.Color = SKColors.White;

        while (_displayNameFont.MeasureText(DisplayName) > Width - _headerHeight * 2)
        {
            _displayNameFont.Size -= 1;
        }

        c.DrawText(DisplayName, _headerHeight + _headerHeight / 3 + 10, _headerHeight / 2f + _displayNameFont.Size / 3, SKTextAlign.Left, _displayNameFont, _informationPaint);
    }
    ...
    private void DrawStatistics(SKCanvas c)
    {
        ...
        if (!string.IsNullOrEmpty(Description))
        {
            _informationFont.Size = 16;
            _informationFont.Typeface = Utils.Typefaces.Description;
            _informationPaint.Color = SKColors.White.WithAlpha(175);

            var fontFromPaint = new SKFont(_informationFont.Typeface, _informationFont.Size);

            Utils.DrawMultilineText(c, Description, Width - 40, 0, SKTextAlign.Center,
                new SKRect(20, outY, Width - 20, Height), fontFromPaint, _informationPaint);

            var lineHeight = fontFromPaint.Size * 1.2f;
            var lines = Utils.SplitLines(Description, fontFromPaint, (Width - 20) - 20);
            if (lines != null)
            {
...
public class IconStat
{
    ...
    private readonly SKPaint _sliderPaint = new()
    {
        IsAntialias = true,
        Color = SKColors.White,
    ...
    {
        _statNameFont.Typeface = Utils.Typefaces.DisplayName;

        while (_statNameFont.MeasureText(_statName) > height * 2 - 40)
        {
            _statNameFont.Size -= 1;
        }

        c.DrawText(_statName, 50, y + 10, SKTextAlign.Left, _statNameFont, _statPaint);

        _statValueFont.Typeface = Utils.Typefaces.BundleNumber;
        _sliderPaint.Color = sliderColor;

        var sliderRight = width - 100 - _statValueFont.MeasureText(_value.ToString());
        c.DrawRect(new SKRect(height * 2, y, Math.Min(width - height, sliderRight), y + 5), _sliderPaint);

        _statPaint.Color = SKColors.White;
        c.DrawText(_value.ToString(), new SKPoint(width - 50, y + 10), SKTextAlign.Right, _statValueFont, _statPaint);
        ...
        c.DrawRect(new SKRect(height * 2, y, Math.Min(height * 2 + sliderWidth, sliderRight), y + 5), _sliderPaint);
    }
}